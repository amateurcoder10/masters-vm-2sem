...
enum        Ops         { HLT,   MOV ,  ADD ,  SUB ,  JNF ,  JMR,   MPC,   IN,   OUT,CMP};
const char *ops_str[] = {"HLT", "MOV", "ADD", "SUB", "JNF", "JMR", "MPC", "IN", "OUT","CMP"};
...
static unsigned getinp(struct PU *pcore)//returns whatever is entered on screen
...
static void putout(struct PU *pcore,  unsigned n)
...
static void op_CMP(struct PU *pcore)
{	unsigned addr1=pcore->pmem[pcore->pc++];
	unsigned addr2=pcore->pmem[pcore->pc++];
	unsigned old,new;

	old = obj_read(pcore, addr1);
	new=obj_read(pcore,addr2);
...
static void op_JNF(struct PU *pcore)
{
	// JNF flag, addr - set PC to addr if flag is not true
...
{
	// IN port, addr - input data from port into reg
	unsigned addr = pcore->pmem[pcore->pc++];//register
	//printf("addr is %d\n",addr);
	unsigned in = getinp(pcore);
	//printf("%d\n",in);
...
static void op_OUT(struct PU *pcore)
{
	...
	putout(pcore,obj_read(pcore, addr));
}
...
unsigned char MEM[MEM_SIZE] = {
	...
	CMP,  reg_(2),   reg_(0),//reg2-reg1;carry flag is set if reg2<reg1
	JNF,num_(1), num_(22),

	MOV,  reg_(2),   addr_(c_v),	//min
	OUT,  reg_(2),	// out[1] = c
	HLT,
	MOV,  reg_(0),   addr_(c_v),	// d = a * b
	OUT, reg_(0),	// out[1] = d
	HLT,
